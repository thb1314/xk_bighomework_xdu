...
for p_idx=1:1:len_p_mat
    ...
    % step(sys)
    [t,x,y] = sim('xiankong02_nolinear.mdl', [0:0.01:10]);
    ...
    sub_title = strcat(sub_title, sprintf(',观测器极点\\lambda_G=3\\lambda,steady value=%.02f',y_steady_value));
    ...
    xlabel('t');
    ylabel('y');
    legend(sprintf('t_s=%.02f\nt_r=%.02f\nt_p=%.02f',ts,tr,tp),'状态观测器y输出曲线','Location','SouthEast');
end
